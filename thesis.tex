...
The classical solutions for $A(z)$ determined in \cite{article:lacroix:2021} give $A(y_i) = J_i$ and $A(z)$ has poles at $y_i+\eta$. This is congruent with the analytic structure of the $R$-matrix $\check R(z-y_i)$. We obtain the Cauchy matrix $C_{ij} = \frac{1}{y_i-y_j+\eta}$, which has inverse
\begin{equation*}
(C^{-1})_{ij} = -(y_j-y_i+\eta) \prod_{k \neq i} \frac{y_j-y_k+\eta}{y_i-y_k} \prod_{k \neq j} \frac{y_k-y_i+\eta}{y_i-y_k}
\end{equation*}
The Cauchy matrix should be seen as a linear map $\mathfrak{gl}_\ell^{(\zeta)} \to \mathfrak{gl}_\ell^{(z)}$. We then have $\operatorname{diag}(X_1,...,X_N)$, which may be understood as a linear map in the opposite direction. Precomposing then yields the Lax matrix as an endomorphism of $\mathfrak{gl}_\ell^{(z)}$ or the defect Lie algebra $\mathfrak{d}$.
...
To this end, we have to determine which module the extended functorial quantum field theory assigns to $E \times S^1$, or more precisely, we have to determine the Drinfeld-type functor
\begin{equation*}
...
To construct a Drinfeld-type functor from this category, we embark to work along the lines of \cite{article:benzvi:2018} and \cite{article:brochier:2017} to define an action of $\widetilde{SL_2(\Z)} \ltimes \ddot S_N$ on the $Y^\vee(\mathfrak{gl}_\ell)^{\otimes N}\text{-}Y^\vee(\mathfrak{gl}_\ell)$-bimodules internal to $Y(\mathfrak{gl}_\ell)\mathsf{Mod}$ resulting from
\begin{equation*}
...
To construct this $\ddot S_N$-action, let us warm up with an $\dot S_N$-action involving the dual Yangian:

\begin{prop}
There is an action of $\dot S_N$ on
\begin{equation*}
Y^\vee(\mathfrak{gl}_\ell) \otimes V_{y_1}^+ \otimes \cdots \otimes V_{y_N}^+
\end{equation*}
via
\begin{equation*}
s_i \mapsto P_{i,i+1} \check R_{i,i+1}(y_i-y_{i+1}), \quad X_1 \mapsto T_1^\vee(z-y_1) R_{1N}(y_1-y_N) \cdots R_{12}(y_1-y_2).
\end{equation*}
\end{prop}
...
We see that
\begin{align*}
&R_{21}(y_2-y_1) T_2^\vee(z-y_2) R_{2N}(y_1-y_N) \cdots R_{23}(y_3-y_2) T_1^\vee(z-y_1) R_{1N}(y_1-y_N) \cdots R_{12}(y_1-y_2) \\
&= R_{21}(y_2-y_1) T_2^\vee(z-y_2) T_1^\vee(z-y_1) R_{2N}(y_1-y_N) \cdots R_{23}(y_3-y_2) R_{1N}(y_1-y_N) \cdots R_{12}(y_1-y_2) \\
&= T_1^\vee(z-y_1) T_2^\vee(z-y_2) R_{21}(y_2-y_1) R_{2N}(y_1-y_N) \cdots R_{23}(y_3-y_2) R_{1N}(y_1-y_N) \cdots R_{12}(y_1-y_2) R_{12}(y_1-y_2) R_{21}(y_2-y_1) \\
&= T_1^\vee(z-y_1) R_{1N}(y_1-y_N) \cdots R_{13}(y_1-y_3) R_{12}(y_1-y_2) R_{21}(y_2-y_1) T_2^\vee(z-y_2) R_{2N}(y_1-y_N) \cdots R_{23}(y_3-y_2),
\end{align*}
so $X_1 X_2 = X_2 X_1$ is fulfilled. Here we have used that $R_{12}(y_1-y_2) R_{21}(y_2-y_1)$ is a scalar.
\end{proof}
...
\begin{equation*}
DY(\mathfrak{gl}_\ell) \otimes V_{y_1}^+ \otimes \cdots \otimes V_{y_N}^+
\end{equation*}
via
\begin{align*}
s_i &\mapsto P_{i,i+1} \check R_{i,i+1}(y_i-y_{i+1}), \\
X_1 &\mapsto T_1^\vee(z-y_1) R_{1N}(y_1-y_N) \cdots R_{12}(y_1-y_2), \\
\bar X_1 &\mapsto R_{21}(y_2-y_1) \cdots R_{N1}(y_N-y_1) \bar T_1^\vee(z-y_1).
\end{align*}
\end{prop}

\begin{proof}
We have to check $\bar X_1 X_2 = X_2 \bar X_1$:
\begin{align*}
&R_{21}(y_2-y_1) \cdots R_{N1}(y_N-y_1) \bar T_1^\vee(z-y_1) R_{21}(y_2-y_1) T_2^\vee(z-y_2) R_{2N}(y_2-y_N) \cdots R_{23}(y_2-y_3) \\
&= R_{21}(y_2-y_1) T_2^\vee(z-y_2) \cdots R_{N1}(y_N-y_1) R_{21}(y_2-y_1) R_{2N}(y_2-y_N) \cdots R_{23}(y_2-y_3) \bar T_1^\vee(z-y_1) \\
&= R_{21}(y_2-y_1) T_2^\vee(z-y_2) R_{2N}(y_2-y_N) \cdots R_{23}(y_2-y_3) R_{21}(y_2-y_1) R_{31}(y_3-y_1) \cdots R_{N1}(y_N-y_1) \bar T_1^\vee(z-y_1)
\end{align*}
where we have used $\bar T_1^\vee(z-y_1) R_{21}(y_2-y_1) T_2^\vee(z-y_2) = T_2^\vee(z-y_2) R_{21}(y_2-y_1) \bar T_1^\vee(z-y_1)$.
\end{proof}
...
\begin{align*}
s X_1 &= \bar X_1^{-1}, \quad s \bar X_1 = X_1^{-1} \\
t X_1 &= X_1, \quad t \bar X_1 = \bar X_1 X_1^{-1}
\end{align*}
...
\begin{align*}
s &: T_1^\vee(z) \mapsto \bar T_1^\vee(z)^{-1}, \quad \bar T_1^\vee(z) \mapsto T_1^\vee(z)^{-1} \\
t &: T_1^\vee(z) \mapsto T_1^\vee(z), \quad \bar T_1^\vee(z) \mapsto \bar T_1^\vee(z) T_1^\vee(z)^{-1}
\end{align*}
...
DY(\mathfrak{gl}_\ell) \otimes V_{y_1}^+ \otimes \cdots \otimes V_{y_N}^+
\end{equation*}
extends to an action of $\widetilde{SL_2(\Z)} \ltimes \ddot S_N$ by letting $s$ act via $s_0$ and $t$ via
\begin{equation*}
t_0 R_{21}(y_2-y_1) \cdots R_{N1}(y_N-y_1).
\end{equation*}
\end{prop}
...
\begin{prop}
There is a homomorphism $DY(\mathfrak{gl}_\ell) \to \operatorname{End}(\C^\ell)[x^{\pm 1}]$ via
\begin{equation*}
\bar T^\vee(z) \mapsto R(z-x), \quad T^\vee(z) \mapsto R(x-z).
\end{equation*}
In particular, we obtain a representation of $DY(\mathfrak{gl}_\ell)$ on $\C^\ell$ for every $x \in \C^\times$ which we denote by $W_x^+$.
\end{prop}

\begin{definition}
The \emph{elliptic transfer matrix} is given by
\begin{equation*}
\tau(z;x) := \operatorname{tr}_{-1,0} R_{0N} \cdots R_{01}, \quad R_{0i} := R_{0i}(z-y_i),
\end{equation*}
acting on
\begin{equation*}
(\overset{-1}{W_x^+} \otimes \overset{1}{V_{y_1}^+} \otimes \cdots \otimes \overset{N}{V_{y_N}^+}) \otimes_{\ddot S_N} \C[y_1,...,y_N].
\end{equation*}
\end{definition}

\begin{prop}
The elliptic transfer matrix $\tau(z;x)$ is a doubly quasi-periodic function.
\end{prop}

\begin{proof}
We have
\begin{align*}
\tau(z;x;y_1,...,y_i+1,...,y_N)
&= \operatorname{tr}_{-1,0} R_{0N}(z-y_N) \cdots R_{0i}(z-y_i-1) \cdots R_{01}(z-y_1) \\
&= \operatorname{tr}_{-1,0} X_i R_{0N}(z-y_N) \cdots R_{0i}(z-y_i) \cdots R_{01}(z-y_1) \\
&= \left( 1 - \frac{\eta}{x-y_i} \right) \tau(z;x;y_1,...,y_N).
\end{align*}
Similarly for $y_i \to y_i+\tau$. Using the unitary $R$-matrix instead, we will get doubly periodic functions.
\end{proof}

\begin{lemma}
We have
\begin{equation*}
\tau_{[1^k]}(z;x) = \widehat{\chi}_{[1^k]}(1) + \sum_j \phi(x,z-y_j) \operatorname{Res}_{z=y_j} \tau_{[1^k]}(z;x).
\end{equation*}
\end{lemma}

\begin{proof}
We match the quasi-periodicity of $\tau(z;x)$ to that of $\phi(w,z)$ and argue that the relevant quotient is a holomorphic elliptic function, hence constant.
\end{proof}
