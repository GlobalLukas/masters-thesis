...
\begin{equation*}
T^\vee(z) := \sum_{ij} t_{ij}^\vee(z) \otimes e_{ij} \in Y^\vee(\mathfrak{gl}_\ell)\llbracket z \rrbracket \otimes \C^\ell, \quad t_{ij}^\vee(z) = \eta \sum_{r \geq 0} t_{ij}^{(-r)} z^r \in Y^\vee(\mathfrak{gl}_\ell)\llbracket z \rrbracket,
\end{equation*}
as well as
\begin{equation*}
...
\end{equation}
define the \emph{canonical element} $\mathcal{R} \in Y^\vee(\mathfrak{gl}_\ell) \otimes Y(\mathfrak{gl}_\ell)$.
\end{definition}
...
\begin{equation*}
V_y^+ \otimes V_{y+\ell \eta}^- \to \C, \quad e_p \otimes e_q \mapsto \delta_{pq}.
\end{equation*}
\end{lemma}
...
\begin{equation*}
\C \to V_{y+\ell \eta}^- \otimes V_y^+, \quad 1 \mapsto \sum_p e_p \otimes e_p.
\end{equation*}
\end{lemma}
...
\begin{proof}
The fact that $V_{y+\ell\eta}^-$ is the left dual for $V_y^+$ follows from the previous lemmata. Twisting by the transposition automorphism of the Yangian turns $V_y^\pm$ into $V_y^\mp$, showing that $V_{y-\ell\eta}^-$ is the right dual by the same argument.
\end{proof}
...
\end{equation*}
with $d_\lambda$ the dimension of the Specht module $S(\lambda)$.
\end{prop}
...
\begin{theorem}
The fusion projectors form a complete set of primitive orthogonal idempotents on $(\C^\ell)^{\otimes k}$ such that
\begin{equation*}
L_{t_\lambda} := \Pi_{t_\lambda} (V_{y-\eta c_1}^+ \otimes \cdots \otimes V_{y-\eta c_k}^+)
\end{equation*}
is a $Y(\mathfrak{gl}_\ell)$-submodule of $V_{y-\eta c_1}^+ \otimes \cdots \otimes V_{y-\eta c_k}^+$ isomorphic to $L(\lambda)_y^t$.
\end{theorem}
...
This \emph{fusion procedure}\index{Fusion procedure} makes it possible to reduce calculations in higher representations to tensor products of the covector representation $V_y^+$. In particular, it allows us to express higher transfer matrices in a very concrete way from a coaction on the auxiliary space $(\C^\ell)^{\otimes k}$:

\begin{definition}
Let $t_\lambda$ be a standard Young tableau of shape $\lambda$ with $k$ boxes and content vector $(c_1,...,c_k)$. We may define a coaction
\begin{align}\label{equation:coaction}
(\C^\ell)^{\otimes k} \to (\C^\ell)^{\otimes k} \otimes Y(\mathfrak{gl}_\ell)\llbracket z^{-1} \rrbracket
\end{align}
via the element
\begin{equation*}
T_{t_\lambda}^g(z-y) := g_k \cdots g_1 T_k(z-y+\eta c_k) \cdots T_1(z-y+\eta c_1).
\end{equation*}
This coaction preserves the subspace $L_{t_\lambda} \subset (\C^\ell)^{\otimes k}$ due to
\begin{equation*}
...
\begin{equation*}
\sum_{i_1,...,i_\ell} \Pi_{t_{[1^\ell]}} (\gamma_{i_1} e_{i_1} \otimes \cdots \otimes \gamma_{i_\ell} e_{i_\ell}) \otimes t_{i_\ell,\ell}(z-\eta(\ell-1)) \cdots t_{i_1,1}(z).
\end{equation*}
...
\begin{proof}
Since $R_{t_\lambda,t_\mu}(z)$ is generically invertible by the previous proposition, we can give the following argument:
\begin{align*}
...
The functional relations between higher transfer matrices come from the so called \emph{fusion relations}\index{Fusion procedure}, whose categorification are certain short exact sequences of comodules. Of particular importance is the following short exact sequence:
\begin{equation*}
...
0 \to C([2,1^{k-1}])_\eta^g \to C([1^k])_\eta^g \otimes C(\ydiagram{1})_0^g \to C([1^{k+1}])_0^g \to 0.
\end{equation*}
of $Y(\mathfrak{gl}_\ell)\llbracket z^{-1} \rrbracket$-comodules.
\end{prop}

\begin{proof}
Choosing column tableaux $t_{[2,1^{k-1}]},t_{[1^k]},t_{[1^{k+1}]}$, we are led to consider the coaction
\begin{equation*}
(\C^\ell)^{\otimes (k+1)} \to (\C^\ell)^{\otimes (k+1)} \otimes Y(\mathfrak{gl}_\ell)\llbracket z^{-1} \rrbracket
\end{equation*}
given by
\begin{equation*}
...
\end{equation*}
due to $H e = (-L^t) (-C^t)^{-1} e$, where $e = (1,...,1)$, so that $(C^t)^{-1} e$ is the vector from (\ref{equation:spectralEq})
\end{proof}
...
\end{equation*}
under the Drinfeld functor.

Let us look at a single site $L(\lambda)_y^t$ with a Young diagram $\lambda$ of $k$ boxes. Take the Specht module $S(\lambda)$ and pull back along the evaluation morphism at $y \in \C$. This yields the $\dot H_k$-module $S(\lambda)_y$ with $y_1,...,y_k$ acting as $y - c_1 \eta,...,y-c_k \eta$ on the basis vector $v_t$ labeled by a standard Young tableau $t$ of shape $\lambda$ with content vector $(c_1,...,c_k)$. We also know that $S(\lambda)_y$ maps to $L(\lambda)_y^t$ under the Drinfeld functor. Note that this is exactly in line with the fusion procedure, taking $y_i \to y - \eta c_i$. We conclude that a single site with spin representation labeled by $\lambda$ is equivalently a \emph{stack of $k$ particles} that sit at relative positions determined by content vectors for standard Young tableaux of shape $\lambda$. A general quantum state is a superposition of such configurations and transforms under $S_N$ in the representation given by the Specht module $S(\lambda)$, \emph{i.e.} the particle wave function of the stack satisfies exchange statistics determined by the Specht module $S(\lambda)$.

What is the correspondence for Hamiltonians? On the spin side, %it is clear that we should again look at the poles of the transfer matrix $\tau^g(z)$, which in our chosen monodromy representation has the form
%\begin{equation*}
%\tau^g(z) = \operatorname{tr}_0 R_{0\lambda^{(N)}}(z-y_N) \cdots R_{0\lambda^{(1)}}(z-y_1),
%\end{equation*}
we consider
\begin{equation*}
...
\end{equation*}
where $\check R_{\lambda^{(i)}\lambda^{(j)}}(y_i-y_j)$ denotes the higher $R$-matrix (\ref{eq:higherRMatrix}) for the Young diagram $\lambda^{(i)}$ and $\lambda^{(j)}$ in the unitary convention. These are the \emph{higher non-local Hamiltonians}. %Thus, we find that $\tau^g(z)$ obtains poles at $z = y_i-\eta c_j$, but these in general might not be simple. The relevant Hamiltonians then are the coefficients of the principal parts at the poles $z = y_i-\eta c_j$. In general, these have the form
%\begin{align*}
...
\begin{proof}
This follows directly from lemma \ref{lemma:keyLemma}.
\end{proof}
...
%\end{align*}
%Using the identity $\Pi_{1...\ell}^- P_{ab} = -\Pi_{1...\ell}^-$ and $\operatorname{tr}_{1...\ell} \Pi_{1...\ell}^- P_{ai} = 1/\ell$ (tba) as well as $\operatorname{tr}_{1...\ell} \Pi_{1...\ell}^- = 1$, we find that the first order term reduces to $N$, the second order term reduces to
%\begin{align*}
...
\hline
$C \times S_*^1 \times [0,1]$ & 4-dimensional & vectors \\
$C \times S_*^1$ & 3-dimensional & modules \\
$D^\times \times S_*^1 \times [0,1]$ & 3-dimensional & modules \\
%$\P^1 \setminus \{ \infty \} \times *$ & 2-dimensional & algebras \\
$D^\times \times S_*^1$ & 2-dimensional & algebras/categories \\
$D^\times \times *$ & 1-dimensional & 2-algebras/2-categories \\
\hline
\end{tabular}
\end{center}
~\\
Here $C$ is a complex curve punctured by ingoing and outgoing points, $D^\times$ denotes the formal punctured disc and $S_*^1$ denotes a circle marked with a basepoint. The module assigned to $D^\times \times S_*^1 \times [0,1]$ will be a bimodule over the algebra assigned to $D^\times \times S_*^1$, which in turn can be computed in terms of the 2-category assigned to $D^\times \times *$. Since the diagrams on cylinders label $D^\times \times *$ with a representation of the Yangian, our functorial quantum field theory should assign the delooping $\mathcal{B}Y(\mathfrak{gl}_\ell)\mathsf{Mod}$ of the monoidal category of modules over the Yangian to $D^\times \times *$. From this, let us compute what should be assigned to $D^\times \times S_*^1$ by cutting $S_*^1$ into a line marked with the basepoint on the interior and an unmarked line as follows:
~\\
\begin{center}
\begin{tikzpicture}
...
\end{center}
~\\
If we insert a $Y(\mathfrak{gl}_\ell)$-module $V$ at the basepoint, the marked line gives the 2-functor that sends $\mathsf{Vect}$ to the category $Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\text{op} \boxtimes Y(\mathfrak{gl}_\ell)\mathsf{Mod}$ and $\C$ to $V^\vee \boxtimes V$, while the unmarked line is nothing but the Hom 2-functor
\begin{equation*}
...
\begin{center}
\begin{tikzcd}
V \arrow{r} \arrow{d}{f} & L' \otimes V \arrow{d}{f} \\
...
\begin{prop}
Let $V$ be a $Y(\mathfrak{gl}_\ell)$-module and equip $Y^\vee(\mathfrak{gl}_\ell)$ with the coadjoint action. Then $V$ is a $Y^\vee(\mathfrak{gl}_\ell)$-comodule internal to $Y(\mathfrak{gl}_\ell)\mathsf{Mod}$ via
\begin{equation*}
...
\end{equation*}
where we have equipped $Y(\mathfrak{gl}_\ell)$ with the adjoint action, making $\rho$ into a $Y(\mathfrak{gl}_\ell)$-linear map. The map $\delta$ satisfies the properties of a coaction due to $(\Delta \otimes \id) = \mathcal{R}_{13} \mathcal{R}_{23}$ and $(\epsilon \otimes \id)(\mathcal{R}) = 1$, which was proven in \cite{article:nazarov:2019}. It remains to show that
\begin{equation*}
...
\begin{equation*}
Y(\mathfrak{gl}_\ell) \otimes L \to Y(\mathfrak{gl}_\ell) \otimes Y^\vee(\mathfrak{gl}_\ell) \to \C
\end{equation*}
...
\item loose morphisms $[N] \dashrightarrow [1]$ are complex cobordisms $\P^1$ with distinct ingoing points $y_1,...,y_N$ and outgoing point $\infty$, %We imagine that the points $y_1,...,y_N$ are secretly labeled by the Young diagram with a single box.
\item squares of the form
\begin{center}
\begin{tikzcd}[column sep=2cm]
{[N]} \arrow[dashed]{r}{(y_1,...,y_N)} \arrow{d}{\sigma} & {[1]} \arrow{d}{\id} \\
{[N]} \arrow[dashed]{r}{(y_{\sigma(1)},...,y_{\sigma(N)})} & {[1]}
\end{tikzcd}
\end{center}
are homotopy classes of paths in the moduli space of pointed complex curves.
\end{enumerate}
...
Let us write $\Gamma: N \to M$ whenever $\Gamma$ is a composition of $N$ by $M$ numbers $N_1,...,N_M$. We can compose $\Gamma: N \to M$ with another composition $\Gamma': M \to L$ and obtain a composition $\Gamma' \circ \Gamma: N \to L$. Given $\Gamma'': N' \to M'$, we may also build a composition $\Gamma \otimes \Gamma'': N+N' \to M+M'$. Define
\begin{equation*}
\underline{\ddot H}_\Gamma := \underline{\ddot H}_{N_1} \otimes \cdots \otimes \underline{\ddot H}_{N_M}, \quad \dot S_\Gamma := \dot S_{N_1} \times \cdots \times \dot S_{N_M}.
\end{equation*}
There are algebra maps $\underline{\ddot H}_{\Gamma' \circ \Gamma} \to \underline{\ddot H}_\Gamma$ defined via the canonical inclusions, as well as $\underline{\ddot H}_\Gamma \to \underline{\ddot H}_\Gamma \otimes \underline{\ddot H}_{\Gamma'}$ defined via the coactions $\underline{\ddot H}_{N_i} \to \underline{\ddot H}_{N_i}[y_i]$. Together, they give an algebra map
\begin{equation*}
\alpha_{\Gamma,\Gamma'}: \underline{\ddot H}_{\Gamma' \circ \Gamma} \to \underline{\ddot H}_\Gamma \otimes \underline{\ddot H}_{\Gamma'}.
\end{equation*}
Finally, we also have isomorphisms $\underline{\ddot H}_{\Gamma \otimes \Gamma''} \cong \underline{\ddot H}_\Gamma \otimes \underline{\ddot H}_{\Gamma''}$. With this in hand, we can give a serious definition for our category of cobordisms:

\begin{definition}
Let $\mathbb{C}\textnormal{ob}$ denote the monoidal (pseudo) double category defined by:
\begin{enumerate}[label=(\roman*)]
...
\item $\mathbb{C}\textnormal{ob}_1$ is the category whose objects are pairs $(\Gamma,U)$, where $\Gamma: N \to M$ and $U$ is a module over $\underline{\ddot H}_\Gamma$ and whose morphisms $(\Gamma,U) \to (\Gamma',U')$ are pairs $(\sigma,\alpha)$, where $\sigma \in \dot S_\Gamma$ and $\alpha$ is a $\sigma$-equivariant map of $\underline{\ddot H}_\Gamma$-modules. Source and target are given by $[N]$ and $[M]$. The monoidal structure is given by $(\Gamma_1,U_2) \otimes (\Gamma_2,U_2) := (\Gamma_1 \otimes \Gamma_2, U_1 \otimes U_2)$. This category represents distributions over holomorphic cobordisms as discussed in subsection \ref{subsection:patch}.
\item The weak composition of $(\Gamma,U): [N] \to [M]$ with $(\Gamma',U'): [M] \to [L]$ is defined to be $(\Gamma' \circ \Gamma, \alpha_{\Gamma,\Gamma'}^* U \otimes U')$.
\end{enumerate}
\end{definition}

\begin{lemma}
Weak composition in $\mathbb{C}\textnormal{ob}$ is weakly associative.
\end{lemma}
...
\end{tikzcd}
\end{center}
It is clear that $(\Gamma'' \circ \Gamma') \circ \Gamma = \Gamma'' \circ (\Gamma' \circ \Gamma)$. We notice that the algebra map
\begin{equation*}
(\alpha_{\Gamma',\Gamma''} \otimes \id) \circ \alpha_{\Gamma,\Gamma'' \circ \Gamma'}: \underline{\ddot H}_{\Gamma'' \circ \Gamma' \circ \Gamma} \to \underline{\ddot H}_{\Gamma'' \circ \Gamma'} \otimes \underline{\ddot H}_\Gamma \to \underline{\ddot H}_{\Gamma''} \otimes \underline{\ddot H}_{\Gamma'} \otimes \underline{\ddot H}_\Gamma
\end{equation*}
coincides with the algebra map
\begin{equation*}
(\id \otimes \alpha_{\Gamma,\Gamma'}) \circ \alpha_{\Gamma' \circ \Gamma,\Gamma''}: \underline{\ddot H}_{\Gamma'' \circ \Gamma' \circ \Gamma} \to \underline{\ddot H}_{\Gamma''} \otimes \underline{\ddot H}_{\Gamma' \circ \Gamma} \to \underline{\ddot H}_{\Gamma''} \otimes \underline{\ddot H}_{\Gamma'} \otimes \underline{\ddot H}_\Gamma,
\end{equation*}
reducing weak associativity to the weak associativity of the usual tensor product.
\end{proof}
...
\begin{align*}
y_i^2
&= \bigg( -\I \hbar X_i \partial_i - i \eta + \eta \sum_{j < i} \theta_{ji} (1-(i \ j)) - \eta \sum_{j > i} \theta_{ij} (1-(i \ j)) \bigg) \bigg( -\I \hbar X_i \partial_i - i\eta \bigg) \\
&= -\hbar^2 (X_i \partial_i)^2 + \I \hbar \eta 2i X_i \partial_i - \I \hbar \eta \sum_{j < i} \theta_{ji} (X_i \partial_i-X_j \partial_j) + \I \hbar \eta \sum_{j > i} \theta_{ij} (X_i \partial_i-X_j \partial_j) + i^2 \eta^2 \\
&= -\hbar^2 (X_i \partial_i)^2 + \I \hbar \eta 2i X_i \partial_i - \I \hbar \eta \sum_{j < i} (1-\theta_{ij}) (X_i \partial_i-X_j \partial_j) + \I \hbar \eta \sum_{j > i} \theta_{ij} (X_i \partial_i-X_j \partial_j) + i^2 \eta^2 \\
&= -\hbar^2 (X_i \partial_i)^2 + \I \hbar \eta 2i X_i \partial_i - \I \hbar \eta \sum_{j < i} (X_i \partial_i-X_j \partial_j) + \I \hbar \eta \sum_{i \neq j} \theta_{ij} (X_i \partial_i-X_j \partial_j) + i^2 \eta^2 \\
&= -\hbar^2 (X_i \partial_i)^2 + \I \hbar \eta (i+1) X_i \partial_i + \I \hbar \eta \sum_{j < i} X_j \partial_j + \I \hbar \eta \sum_{i \neq j} \theta_{ij} (X_i \partial_i-X_j \partial_j) + i^2 \eta^2,
\end{align*}
so
\begin{align*}
\sum_i y_i^2 &= -\hbar^2 \sum_i (X_i \partial_i)^2  + \I \hbar \eta \sum_{i \neq j} \theta_{ij} (X_i \partial_i-X_j \partial_j) - \eta^2 \frac{N(N+1)(2N+1)}{6}.
\end{align*}
...
&= \sum_i \bigg( \I \hbar X_i \partial_i - 2\eta \sum_{i \neq j} \theta_{ij} \bigg) \bigg( \I \hbar X_i \partial_i - 2\eta \sum_{i \neq j} \theta_{ij} \bigg) \\
&= -\hbar^2 \Delta - 4\I\hbar\eta \sum_{i \neq j} \theta_{ij} X_i \partial_i - 2\I\hbar\eta \sum_{i \neq j} \theta_{ij} \theta_{ji} + 4\eta^2 \sum_i \sum_{i \neq j} \sum_{i \neq k} \theta_{ij} \theta_{ik}
\end{align*}
Is the 2 wrong? Then we would have
\begin{equation*}
-\hbar^2 \Delta - \frac{\I\hbar\eta}{2} \sum_{i \neq j} (\theta_{ij}-\theta_{ji}+1) X_i \partial_i - \I\hbar\eta \sum_{i \neq j} \theta_{ij} \theta_{ji} + \eta^2 \sum_i \sum_{i \neq j} \sum_{i \neq k} \theta_{ij} \theta_{ik}
\end{equation*}
The last term gives
\begin{align*}
\sum_i \sum_{i \neq j} \sum_{i \neq k} \theta_{ij} (1-\theta_{ki}) = (N-1) \sum_{i \neq j} \theta_{ij} - \sum_{i \neq j \neq k} \theta_{ij} \theta_{ki} - \sum_{i \neq j} \theta_{ij} \theta_{ji} = \frac{N(N-1)^2}{2} - ? - \sum_{i \neq j} \theta_{ij} \theta_{ji}.
\end{align*}
\end{proof}
