...
\end{equation*}
This looks awfully close to the Lax matrix (\ref{equation:RSLaxMatrix}) of the rational Ruijsenaars-Schneider model! Iterating and combining with the formula for the quantum determinant from corollary \ref{corollary:quantumDet} finally gives the spectral equation, resembling a Cayley-Hamilton-type identity:
...
\end{equation*}
by the fusion procedure with corresponding projector $\Pi_{t_\lambda}$, up to a choice of tableau $t_\lambda$ with content vector $(c_1,...,c_N)$. On the particle side, the projector $\Pi_{t_\lambda}$ similarly corresponds to a projector and the positions of particles get restricted to $y_i = y + \eta c_i$. Physically, this means that spins in higher representations labeled by $\lambda$ do not correspond to single particles, but to a bound state of $N$ particles, where $N$ is the number of boxes of the Young diagram $\lambda$. The exact correspondence is found in \cite{article:arakawa:1999}.
...
The gluing of curves becomes tensoring over $Y(\mathfrak{gl}_\ell)$. For this, we might need to compute the coend $\int^{y_2-y_1} L(\ydiagram{1})_{y_1}^t \otimes_{Y(\mathfrak{gl}_\ell)} L(\ydiagram{1})_{y_2}^t$. I would expect $L(\ydiagram{1})_{y_1}^t \otimes_{Y(\mathfrak{gl}_\ell)} L(\ydiagram{1})_{y_2}^t$ to be the trivial representation except at $y_2-y_1 = \pm \eta$, where we get the symmetric and antisymmetric representation. As a $U(\mathfrak{gl}_\ell)$-module, their sum would be the coadjoint representation, as one might expect from this coend.
