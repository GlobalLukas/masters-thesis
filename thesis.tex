...
\end{pmatrix}.
\end{equation*}
We have $\operatorname{qdet} T(z) = \prod_k \frac{z-y_k+\eta}{z-y_k}$ (tba: sign?), so that $\operatorname{Res}_{z=y_j} \operatorname{qdet} T(z) = \eta b_j$. On the other hand, we have $L = CH \Leftrightarrow H = L^t (C^t)^{-1}$, where $C_{ij} = \frac{\eta}{y_i-y_j+\eta}$ is the Cauchy matrix and $H = \operatorname{diag}(H_1,...,H_N)$.
This implies
\begin{equation*}
\eta e_\ell(g)
\begin{pmatrix}
...
Let us look at a single site labeled by a Young diagram $\lambda$ of $k$ boxes. Take the Specht module $S(\lambda)$, twist it by the sign representation to obtain $S(\lambda)^s$ and pull back along the evaluation morphism (\ref{equation:degHeckeEval}). This yields the $\dot H_k$-module $S(\lambda)^s[y]$ where $y_i$ acts as $y+\eta Y_i$, reducing to $y+\eta c_i(t)$ on the basis vector $v_t$ labeled by a standard Young tableau $t$ of shape $\lambda$.
...
\end{enumerate}
However, solving the Wronskian equation \cite{book:arutyunov:betheAnsatz} for the $N=4$ $\mathfrak{gl}_2$ spin chain with generic inhomogeneities and performing the fusion procedure as well as going to the homogeneous limit yields the solution
\begin{equation*}
...
\begin{equation*}
E_\Gamma := E[y_1] \otimes \cdots \otimes E[y_N][(y_i-y_j)^{-1},(y_i-y_j-\eta)^{-1}].
\end{equation*}
...
\begin{equation*}
R^E(y_1-y_2) := R_{24}(y_1-y_2-\ell\eta) R_{13}(y_1-y_2+\ell\eta) \in \operatorname{End} E[y_1] \otimes E[y_2],
\end{equation*}
...
\end{align*}
This defines a right $\underline{\dot H}_\Gamma$-module structure on $E_\Gamma$, where the permutation $(i \ j)$ acts via the braiding $(i \ j) R^E(y_i-y_j)$.
...
\begin{center}
\begin{tikzcd}[column sep=2cm]
E_\Gamma \otimes_{S_\Gamma} U \arrow{r}{R_{i,i+1}(y_i-y_{i+1})} & E_\Gamma \otimes_{S_\Gamma} U \arrow{r}{\id \otimes f} & E_\Gamma \otimes_{S_\Gamma} U',
\end{tikzcd}
\end{center}
Both maps are $s_i$-equivariant maps of $Y(\mathfrak{gl}_\ell)$-modules, so their composition will be an $\id$-equivariant map of $Y(\mathfrak{gl}_\ell)$-modules, meaning it is $R$-linear. tba
