...
\begin{theorem}
The Hamiltonian $\delta^{-1} C_1 \delta \in \delta^{-1} S\ddot H_N \delta$ acts on $D_{\ell,N}(\C[X_1^{\pm 1},...,X_N^{\pm 1}])$ in the same way as $-N\eta -\frac{\eta}{2} \sum_i G_i$ when $\hbar = 0$.
\end{theorem}
...
The diagrams on cylinders suggest that we should assign the delooping $\mathcal{B}Y(\mathfrak{gl}_\ell)\mathsf{Mod}$ to the direct product of a formal punctured disc and a point $D^\times \times *$. But what should be assigned to $D^\times \times S_*^1$, where $S_*^1$ is a circle with basepoint? By general abstract arguments from topological quantum field theory, a sensible guess would be the vertical trace
\begin{equation*}
