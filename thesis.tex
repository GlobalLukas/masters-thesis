...
To define the symmetric monoidal double category $\mathsf{Cob}_{0,2,4}$, we start with some preliminary definitions:
...
We want a functor mapping the $i$th braid to $\check R_{i,i+1}(y_i-y_{i+1}) s_i$:
\begin{center}
\begin{tikzcd}[column sep=4cm]
{[0]} \arrow{d}{\id} \arrow{r}{(\P^1;\infty,y_1,...,y_i,y_{i+1},...,y_N)} & {[N]} \arrow{d}{s_i} \\
{[0]} \arrow[swap]{r}{(\P^1;\infty,y_1,...,y_{i+1},y_i,...,y_N)} & {[N]}
\end{tikzcd}
$\quad \mapsto \quad$
\begin{tikzcd}[column sep=4cm]
{\C[\eta]} \arrow{d}{\id} \arrow{r}{L(\ydiagram{1})_{y_1}^t \otimes \cdots \otimes L(\ydiagram{1})_{y_N}^t} & Y(\mathfrak{gl}_\ell)^{\otimes N} \arrow{d}{s_i} \\
{\C[\eta]} \arrow[swap]{r}{L(\ydiagram{1})_{y_1}^t \otimes \cdots \otimes L(\ydiagram{1})_{y_N}^t} & Y(\mathfrak{gl}_\ell)^{\otimes N}
\end{tikzcd}
\end{center}
and mapping the twist operators to ??. Mapping them to $\check H_i$ does not commute with the Yangian! Hence they should be mapped to separate operators. Possibly, one should look at the representations $L(\ydiagram{1})_{y,X}^t$ of $L(\mathfrak{gl}_\ell) \# Y(\mathfrak{gl}_\ell)$.

The gluing of curves becomes tensoring over $Y(\mathfrak{gl}_\ell)$. What is $\int^y (L(\ydiagram{1})_y^t)^\vee \otimes_{Y(\mathfrak{gl}_\ell)} L(\ydiagram{1})_y^t$? Probably just $\C[\eta]$.
