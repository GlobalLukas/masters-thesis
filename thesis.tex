...
The gluing of curves becomes tensoring over $Y(\mathfrak{gl}_\ell)$. For this, we might need to compute the coend $\int^{y_2-y_1} L(\ydiagram{1})_{y_1}^t \otimes_{Y(\mathfrak{gl}_\ell)} L(\ydiagram{1})_{y_2}^t$. I would expect $L(\ydiagram{1})_{y_1}^t \otimes_{Y(\mathfrak{gl}_\ell)} L(\ydiagram{1})_{y_2}^t$ to be the trivial representation except at $y_2-y_1 = \pm \eta$, where we get the symmetric and antisymmetric representation. As a $U(\mathfrak{gl}_\ell)$-module, their sum would be the coadjoint representation, as one might expect from this coend, but the Yangian action is non-trivial.

In the above, the loose identity morphisms for $[1]$ are $[2]$-pointed curves. For $[N]$, they should be disjoint unions of such curves, \emph{i.e.} we have to allow non-connected curves.

What happens if we consider $\Sigma$ that are not just cylinders, but any compact oriented surface with boundaries? These can no longer be interpreted as giving families of curves, but it suggests that any curve has to map to some Frobenius algebra object in $Y(\mathfrak{gl}_\ell)^{\otimes N}\mathsf{Mod}$. To obtain two circles, we would have to consider multiple possible tangent vectors at each point $y_i$. Does this make sense for non-fundamental Young diagrams? In general, we would expect that the reducibility phenomena in the representation theory of the Yangian are also represented on the geometric side when points coalesce into bound states with $y_2=y_1 \pm \eta$. For $\omega$, this would mean that a zero eats a pole. We may label the curves by standard tableau and a choice of tangent vector at any of the particles in the bound state. But how to do this for non-zero genus?
