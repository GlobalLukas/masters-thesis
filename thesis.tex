...
\begin{definition}
Due to this fact, $\tau_{[1^\ell]}^g(z)$ deserves the name \emph{$g$-twisted quantum determinant}\index{Quantum determinant}. We also write $\operatorname{qdet}^g T(z) := \tau_{[1^\ell]}^g(z)$ and $\operatorname{qdet} T(z) := \tau_{[1^\ell]}^1(z)$.
\end{definition}

\begin{prop}
The quantum determinant $\operatorname{qdet} T(z)$ is a group-like element.
\end{prop}
...
\end{align*}
For this to be valid, it remains to check that $R_{t_\lambda,t_\mu}(z-w)$ preserves the subspace $L_{t_\lambda} \otimes L_{t_\mu}$. Indeed, we have
\begin{equation*}
...
\begin{proof}
Since the quantum determinant is a group-like element, we find that the quantum determinant acting on $LD_{\ell,N}(\C[X_1^{\pm 1},...,X_N^{\pm 1}])$ is given by the product of elementary quantum determinants $1-\frac{\eta}{z-y_i}$, which becomes
\begin{equation*}
\operatorname{qdet} T(z) = \frac{\Delta(z-\eta)}{\Delta(z)}, \quad \Delta(z) = \prod_i (z-y_i).
\end{equation*}
