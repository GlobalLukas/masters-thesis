...
\begin{lemma}
There is a filtered basis $X_1,X_2,...$ of $Y(\mathfrak{gl}_\ell)$ and a filtered basis $X_1',X_2',... \in \hat Y^\vee(\mathfrak{gl}_\ell)$ with $\langle X_i,X_j' \rangle = \delta_{ij}$.
\end{lemma}
...
The elements
\begin{equation*}
\mathcal{R}_N := \sum_{\deg X_i \leq N} X_i' \otimes X_i \in \hat Y^\vee(\mathfrak{gl}_\ell) \otimes F_N Y(\mathfrak{gl}_\ell)
\end{equation*}
...
Note that the parametrizations of the boundary components of an extended surface $\Sigma$ may equivalently be thought of as an embedding of a disc $D^2$ into the surface that results from gluing the boundary component of $\Sigma$ closed. It will thus be enough to look at the following category of embeddings:

\begin{definition}
Let $\mathsf{Mfld}_\text{fr}^2$ be the $(\infty,1)$-category whose objects are framed 2-manifolds, whose 1-morphisms are smooth embeddings, whose 2-morphisms are isotopies between embeddings, and whose higher morphisms are higher homotopies between these.
\end{definition}

Given the data of an abelian rigid braided monoidal category, \cite{article:benzvi:2018} constructs a 2-functorial field theory on $\mathsf{Mfld}_\text{fr}^2$ via factorization homology. To accomplish this, we need an abelian braided rigid tensor category as the (framed) $E_2$-algebra of coefficients. Such a category is readily provided by a certain category of $Y(\mathfrak{gl}_\ell)$-modules:

\begin{definition}
Consider $\eta$ as a non-zero purely imaginary number. Let $Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\R$ be the subcategory of $Y(\mathfrak{gl}_\ell)\mathsf{Mod}$ generated by modules of the form
\begin{equation*}
...
\item Braidings for generators: Define the braidings
\begin{equation*}
c_{\pm\pm}: V_{y_1}^\pm \otimes V_{y_2}^\pm \to V_{y_2}^\pm \otimes V_{y_1}^\pm
\end{equation*}
...
\end{matrix}
\end{equation*}
We check with Mathematica that this reduces to $P_{1,1'} P_{2,2'}$ in the limit $y_1 - y_2 \to 0$.
\end{itemize}
...
\item Case “$-+$”: This is similarly the identity.
\item Case “$--$”: Similarly.
\end{itemize}
...
\begin{lemma}
There exist finitely many configurations $(y_1,...,y_N)$ such that $x \in F_N Y(\mathfrak{gl}_\ell)$ is zero if and only if $x$ acts as zero on all $V_{y_1}^+ \otimes \cdots \otimes V_{y_N}^+$.
\end{lemma}
...
\begin{prop}
The dual Yangian $Y^\vee(\mathfrak{gl}_\ell)$ is the coend $\int^{X \in Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\R} X \otimes X^\vee$.
\end{prop}

\begin{proof}
Let $\mathsf{M}_N$ be the full subcategory spanned by $V_{y_1}^+ \otimes \cdots \otimes V_{y_N}^+$ for the finitely many configurations $(y_1,...,y_N)$ of the previous lemma. Let
\begin{equation*}
E_N := \bigoplus_{M \in \mathsf{M}_N} M \otimes M^\vee \in Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\R.
\end{equation*}
with the special element $1_N: \C[\eta] \to E_N$ representing the sum of identities. By the previous lemma, we can identify $F_N Y(\mathfrak{gl}_\ell)$ as a subspace of $E_N$ via the map $Y(\mathfrak{gl}_\ell) \to E_N$. Further let $L := \int^{X \in Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\R} X \otimes X^\vee$. The coactions of the dual Yangian give a map $\psi: L \to Y^\vee(\mathfrak{gl}_\ell)$ that commutes with the coactions $E_N \to L \otimes E_N$ and $E_N \to Y^\vee(\mathfrak{gl}_\ell) \otimes E_N$. We may look at the image of $1_N$
\begin{equation*}
\widetilde{\mathcal{R}}_N: \C[\eta] \to E_N \to L \otimes L \otimes F_N Y(\mathfrak{gl}_\ell \subseteq L \otimes E_N,
\end{equation*}
which under $\psi \otimes \id$ gets mapped to $\mathcal{R}(1 \otimes 1_N)$, which is nothing but $\mathcal{R}_N \in Y^\vee(\mathfrak{gl}_\ell) \otimes F_N Y(\mathfrak{gl}_\ell)$. In the limit, we obtain an element
\begin{equation*}
\widetilde{\mathcal{R}} \in \varprojlim_N L \otimes E_N
\end{equation*}
...
\begin{equation*}
\mathcal{Z} := \int_{(-)} Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\R: \mathsf{Mfld}_\textnormal{fr}^2 \to \mathsf{BrMonCat}.
\end{equation*}
This functor sends $\Sigma_{g,1+N}$ to the category of $Y^\vee(\mathfrak{gl}_\ell)^{\otimes (2g+N)}$-modules internal to $Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\R$.
\end{corollary}

The sphere with one marked boundary component (\emph{i.e.} the disc) gets mapped to $Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\R$. We may look at embeddings of $N$ discs into it. Isotopies between such embeddings modulo homotopy form the framed braid group $B_N \ltimes \Z^N$. This equips the functor
\begin{equation*}
V_1 \boxtimes \cdots \boxtimes V_N \mapsto V_1 \otimes \cdots \otimes V_N
\end{equation*}
with an $S_N$-equivariant action of $B_N \ltimes \Z^N$ via the braiding and the balancing.

\begin{prop}
The induced action of the framed braid group on fundamental monodromy representations is given by the action of $\ddot H_N$.
\end{prop}
...
The 2-functor $\mathcal{Z}$ sends the torus with one marked boundary component to $DY(\mathfrak{gl}_\ell)$-modules internal to $Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\R$.
