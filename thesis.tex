...
\end{equation*}
under finite (co)limits. Here, the superscript $(-)^\pm$ indicates either $V_{y_i}^+$, in which case $y_i \in \R + 2\ell\eta\Z$, or $V_{y_i}^-$, in which case $y_i \in \R + \ell\eta + 2\ell\eta\Z$.
\end{definition}
...
\item Case “$-+$”: $y_1-y_2 \neq -\eta k \pm \eta$ for $k = 0,...,\ell-2$,
\item Case “$--$”: This case is more complicated, since we have possible singularities when $d_j-c_i=\pm 1$. Note however, that when $d_j-c_i=+1$, then we will find $i',j'$ with $d_{j'}-c_{i'}=-1$, and vice versa. Thus, for every singularity, we have a corresponding inverse singularity that cancels it.
\end{itemize}
...
\begin{proof}
Let $L := \int^X X \otimes X^\vee$. The left coactions of $Y^\vee(\mathfrak{gl}_\ell)$ on every $X$ yield a map $\psi: L \to Y^\vee(\mathfrak{gl}_\ell)$. Let
\begin{equation*}
M_N := V_0^+ \otimes \cdots \otimes V_0^+, \quad m_N := e_1 \otimes \cdots \otimes e_1, \quad E_N := \underline{\operatorname{End}}(M_N) \cong M_N \otimes M_N^\vee.
\end{equation*}
We have natural coactions $E_N \to L \otimes E_N$ and $E_N \to Y^\vee(\mathfrak{gl}_\ell) \otimes E_N$ such that the following diagrams commute:
\begin{center}
\begin{tikzcd}
E_N \arrow{r} \arrow{dr} & L \otimes E_N \arrow{d}{\psi \otimes \id} \\
& Y^\vee(\mathfrak{gl}_\ell) \otimes E_N
\end{tikzcd}
\begin{tikzcd}
E_{N+1} \arrow{r} \arrow{d}{\operatorname{tr}_N} & L \otimes E_{N+1} \arrow{d}{\id \otimes \operatorname{tr}_N} \\
E_N \arrow{r} & L \otimes E_N
\end{tikzcd}
\begin{tikzcd}
E_{N+1} \arrow{r} \arrow{d}{\operatorname{tr}_N} & Y^\vee(\mathfrak{gl}_\ell) \otimes E_{N+1} \arrow{d}{\id \otimes \operatorname{tr}_N} \\
E_N \arrow{r} & Y^\vee(\mathfrak{gl}_\ell) \otimes E_N
\end{tikzcd}
\end{center}
Let $\widetilde{\mathcal{R}}_N \in L \otimes E_N$ denote the image of $1_{M_N} \in E_N$. We know that $\psi \otimes \id$ sends $\widetilde{\mathcal{R}}_N$ to $\mathcal{R}(1 \otimes 1_{M_N}) \in Y^\vee(\mathfrak{gl}_\ell) \otimes E_N$. Now show that
\begin{equation*}
\varprojlim_N L \otimes E_N = L \operatorname{\hat{\otimes}} Y(\mathfrak{gl}_\ell), \quad \varprojlim_N Y^\vee(\mathfrak{gl}_\ell) \otimes E_N = Y^\vee(\mathfrak{gl}_\ell) \operatorname{\hat{\otimes}} Y(\mathfrak{gl}_\ell)
\end{equation*}
and that $\widetilde{\mathcal{R}} = (\widetilde{\mathcal{R}}_N)_N$ gets mapped to $\mathcal{R}$. The element $\widetilde{\mathcal{R}}$ then witnesses that the composition $Y(\mathfrak{gl}_\ell) \operatorname{\hat{\otimes}} L \to Y(\mathfrak{gl}_\ell) \operatorname{\hat{\otimes}} Y^\vee(\mathfrak{gl}_\ell) \to \C[\eta]$ is a non-degenerate pairing, implying that $\psi$ is an isomorphism (tba).
\end{proof}

\begin{corollary}
Factorization homology gives a 2-functor
\begin{equation*}
\mathcal{Z} := \int_{(-)} Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\R: \mathcal{ES} \to \mathcal{M}on_\textnormal{sym}.
\end{equation*}
that sends $\Sigma_{1+N}^g$ to the category of $Y^\vee(\mathfrak{gl}_\ell)^{\otimes (2g+N)}$-modules internal to $Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\R$.
\end{corollary}
