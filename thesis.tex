...
\item Braidings for generators: Let $\check R_+(z) := P \check R(z) = \frac{z}{z-\eta} - \frac{\eta}{z-\eta} P$ and $\check R_-(z) := \frac{z}{z-\eta} P - \frac{\eta}{z-\eta} P^t$. Define the braidings
\begin{align*}
...
\end{align*}
One may check that $c_{\pm\mp}$ and $c_{--}$ come about by dualizing the legs of $c_{++}$. In particular, we check that the braiding is well-defined and invertible for all admissible values of $y_1$ and $y_2$.
...
The map
\begin{equation*}
\delta: V_{y_1}^+ \otimes \cdots \otimes V_{y_N}^+ \to Y^\vee(\mathfrak{gl}_\ell) \otimes V_{y_1}^+ \otimes \cdots \otimes V_{y_N}^+, \quad v \mapsto T_N^\vee(y_N) \cdots T_1^\vee(y_1)
\end{equation*}
