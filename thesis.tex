...
\item We construct a functorial quantum field theory representing four-dimensional Chern-Simons theory on $\P^1 \times S^1 \times [0,1]$ and show how this yields the rational Ruijsenaars-Schneider and Heisenberg models upon dimensional reduction by $S^1$ or $\P^1$, respectively.
\item We extrapolate our description to four-dimensional Chern-Simons theory on $E \times S^1 \times [0,1]$, where $E$ is an elliptic curve, making a connection to the elliptic Ruijsenaars-Schneider model.
\end{itemize}

The remaining chapters of this thesis are structured as follows. Chapter \ref{chapter:integrability} gives a full review from the basics of integrability to the state of the art of the pertinent models and establishes the setup that frames our discussion. Chapter \ref{chapter:duality} begins with a detailed discussion of the recent appearance of the Lax matrix of the rational Ruijsenaars-Schneider model in the functional relations for transfer matrices of the Heisenberg model used to derive the spectral equation whose solutions yield the energy spectrum of the Heisenberg model. We then move on to describe the mathematical underpinnings of generalized Schur-Weyl duality and how it gives rise to quantum-classical duality. The end of chapter \ref{chapter:duality} is dedicated to explicating $S$-duality, showing how the $S$-dual models, \emph{i.e.} the trigonometric Gaudin model and the trigonometric Calogero-Moser model are also related by quantum-classical duality. Chapter \ref{chapter:geometry} then continues with a description of the geometry behind the mathematical structures of chapter \ref{chapter:duality}, constructing a functorial quantum field theory extending the Drinfeld functor. Finally, we argue why this functorial quantum field theory represents four-dimensional Chern-Simons theory. After the conclusion (chapter \ref{chapter:conclusion}), we give supplementary material on Young diagrams (appendix \ref{appendix:youngdiagrams}) as well as results on Dunkl operators for the trigonometric Calogero-Moser model (appendix \ref{appendix:dunklOperators}) and an exposition on moduli spaces of curves (appendix \ref{appendix:moduliSpaces}).
...
What is the correspondence for Hamiltonians? On the spin side, the Hamiltonians are given by the same formula, except that we use fused $R$-matrices. On the particle side, these correspond to fused $x$-operators.
...
\item 1-morphisms $[N] \to [M]$ are stable complex curves marked with $N$ ingoing and $M$ outgoing ordered points. Composition is defined by gluing outgoing to ingoing points and identities are given by $\P^1$ marked with $0$ and $\infty$,
...
By general algebro-geometric arguments, a quasi-coherent sheaf $U$ on $M_{0,N+1}$ is equivalently an $S_N \times \C$-equivariant $\C[y_1,...,y_N]_\delta$-module. This means that $U$ comes equipped with an action of the group $S_N \times \C$ such that
\begin{equation*}
...
\end{align*}
We may fix the $\C^\times$-symmetry by requiring that $ca^{-1} = \eta$. The only remaining symmetry is $S_N \times \C$. The requirement that $\omega$ should have $N$ simple poles and zeros away from infinity implies $y_i-y_j \neq 0,\eta$.
\end{proof}

\begin{remark}
We may add boundary points to $M_{0,1+N}^\omega$ by the fusion procedure. Such boundary points will be labeled by trees with standard Young tableaux at each internal edge.
\end{remark}
...
Composition of sheaves should then come from the map $\dot H_N \otimes \dot H_M \otimes \dot H_2 \to \dot H_{N+M}$ sending $s_2 \in H_2$ to $(1 \ N+1) \cdots (N \ M)$ and $y_i$ to $y_i+y_1'$ for $i=1,...,N$ and $y_1' \in \dot H_2$.
...
\end{equation*}
that sends the formal punctured disk $D^\times$ to $Y(\mathfrak{gl}_\ell)\mathsf{Mod}$ and on Hom categories
\begin{equation*}
...
\end{equation*}
equips $V_y^+$ with the structure of a $Y^\vee(\mathfrak{gl}_\ell)$-comodule internal to $Y(\mathfrak{gl}_\ell)$.
\end{lemma}
...
\begin{center}
\begin{tikzcd}
{[N]} \arrow{d}{\sigma} \arrow{r}{U} & {[1]} \arrow{d}{\id} \\
{[N]} \arrow{r}{V} & {[1]}
\end{tikzcd}
\end{center}
equipped with $\sigma$-equivariant morphisms $\alpha: U \to V$ of quasi-coherent sheaves, which we may equip with elements of $\Z^N$ to represent Dehn twists,
...
We may insert boundary conditions at the poles of $\omega$. These are given by Frobenius algebras $A$ internal to $Y(\mathfrak{gl}_\ell)\mathsf{Mod}$ with a quantum moment map $Y^\vee(\mathfrak{gl}_\ell) \to A$.

\section{Quantum-classical duality in terms of four-dimensional Chern-Simons theory}

It is natural to ask which physical theory gives rise to the functorial quantum field theory $\overline{\mathcal{Z}}_0$ we have defined. Considering that we are looking at topological surfaces as tight morphisms and (sheaves on moduli spaces of) complex curves as loose morphisms, we are already given a hint that the theory should have two holomorphic and two topological dimensions. The functors should assign holonomies of some flat connection on $S^1 \times \R$ to the braids. Furthermore, the Yangian provides the Hilbert space of the formal punctured disk $D^\times$. These observations all point to four-dimensional Chern-Simons theory.
...
\section{Musings on the elliptic Ruijsenaars-Schneider model}

We have seen how the rational Ruijsenaars-Schneider model arises from the dynamics of order defects in four-dimensional Chern-Simons theory on $\P^1 \times S^1 \times [0,1]$. One might hope that the functorial quantum field theory we constructed has an extension to complex curves of higher genus. In the case of an elliptic curve $E = \C/\Lambda$, we can then look at $E \times S^1 \times [0,1]$ and hope for a description of the dynamics of the 1-form
\begin{equation*}
\omega = K \prod_i \frac{\sigma(z-y_i-\eta)}{\sigma(z-y_i)} dz
\end{equation*}
in terms of the elliptic Ruijsenaars-Schneider model.

To this end, we have to determine which bimodule the extended functorial quantum field theory assigns to $E \times S^1$, or more precisely, we have to determine the Drinfeld-type functor
\begin{equation*}
\mathsf{Qcoh}(M_{1,1+N}^\omega) \to Y^\vee(\mathfrak{gl}_\ell)^{\otimes N}\text{-}Y^\vee(\mathfrak{gl}_\ell)\mathsf{Bimod}_{Y(\mathfrak{gl}_\ell)\mathsf{Mod}},
\end{equation*}
...
To construct a Drinfeld-type functor from this category, we resort to \cite{article:brochier:2017} to define an action of $\widetilde{SL_2(\Z)} \ltimes \ddot S_N$ on the $Y^\vee(\mathfrak{gl}_\ell)^{\otimes N}\text{-}Y^\vee(\mathfrak{gl}_\ell)$-bimodules internal to $Y(\mathfrak{gl}_\ell)\mathsf{Mod}$ resulting from
\begin{equation*}
V_1 \boxtimes \cdots \boxtimes V_N \mapsto EY(\mathfrak{gl}_\ell) \otimes V_1 \otimes \cdots \otimes V_N.
\end{equation*}
Here $EY(\mathfrak{gl}_\ell)$ denotes the elliptic Yangian:

\begin{definition}
The elliptic Yangian $EY(\mathfrak{gl}_\ell)$ is given by $Y^\vee(\mathfrak{gl}_\ell) \otimes Y^\vee(\mathfrak{gl}_\ell)$ with cross relations
\begin{equation*}
T_1^\vee(z) R_{21}(w-z) \bar T_2^\vee(w) R_{12}(z-w) = R_{21}(w-z) \bar T_2^\vee(w) R_{12}(z-w) T_1^\vee(z)
\end{equation*}
in $Y^\vee(\mathfrak{gl}_\ell) \otimes Y^\vee(\mathfrak{gl}_\ell) \otimes \overset{1}{\operatorname{End} V} \otimes \overset{2}{\operatorname{End} V}$. Here $T^\vee(z)$ and $\bar T^\vee(w)$ respectively denote the generator matrix of the first and second tensorand.
\end{definition}
...
\begin{prop}
There is an action of $\widetilde{SL_2(\Z)}$ on $EY(\mathfrak{gl}_\ell)$ via
\begin{align*}
T T^\vee(z) &= \bar T^\vee(w), \quad T \bar T^\vee(w) = \bar T^\vee(w) T^\vee(z)^{-1} \bar T^\vee(w)^{-1} \\
S T^\vee(z) &= T^\vee(w), \quad S \bar T^\vee(w) = \bar T^\vee(w) T^\vee(z)^{-1}.
\end{align*}
\end{prop}

\begin{definition}
The \emph{elliptic degenerate affine Hecke algebra} $E\dot H_N$ is the algebra
\begin{equation*}
SL_2(\Z) \ltimes \ddot S_N \ltimes \C[y_1,...,y_N] \otimes \mathcal{O}(\mathbb{H}).
\end{equation*}
\end{definition}
...
\begin{equation*}
ED_{\ell,N}: E\dot H_N \mathsf{Mod} \to Y^\vee(\mathfrak{gl}_\ell)^{\otimes N}\text{-}Y^\vee(\mathfrak{gl}_\ell)\mathsf{Bimod}_{Y(\mathfrak{gl}_\ell)\mathsf{Mod}}.
\end{equation*}
\end{theorem}

What does a transfer matrix look like for this extended spin chain? If we just consider a trace over a product of $R$-matrices then we won't gain anything new. We have to look at the elliptic generators as well and trace over them. In the end, we have to obtain an elliptic function as transfer matrix, which we may then examine for fusion relations.

By Akhiezer "Elements of the Theory of Elliptic Functions" page 40, we have
\begin{lemma}
Let $f(z)$ be an elliptic function. Then we may write $f$ as
\begin{equation*}
f(z) = C + \sum_k A_k \zeta(z-y_k).
\end{equation*}
\end{lemma}
In our case, we want to be able to write
\begin{equation*}
\tau_{[1^k]}^g(z;w) = \widehat{\chi}_\lambda(g) + \sum_j \Phi(w,z-y_j) \operatorname{Res}_{z=y_j} \tau_{[1^k]}^g(z;w)
\end{equation*}
to obtain
\begin{equation*}
\Phi(w,y_i-y_j-\eta) \prod_{k \neq j} \frac{\sigma(y_j-y_k+\eta)}{\sigma(\eta)\sigma(y_j-y_k)} e^{-\I \hbar \partial_j}
\end{equation*}

\begin{definition}
The \emph{elliptic transfer matrix} is given by
\begin{equation*}
\tau^\text{ell}(z) := \operatorname{tr}_{-1,0} R_{0N} \cdots R_{01}, \quad R_{0i} := R_{0i}(z-y_i),
\end{equation*}
acting on
\begin{equation*}
(\overset{-1}{EY(\mathfrak{gl}_\ell)} \otimes \overset{1}{V_{y_1}^+} \otimes \cdots \otimes \overset{N}{V_{y_N}^+}) \otimes_{\ddot S_N} \C[y_1,...,y_N].
\end{equation*}
\end{definition}

\begin{prop}
The elliptic transfer matrix $\tau^\textnormal{ell}(z)$ is a doubly quasi-periodic function.
\end{prop}

\begin{proof}
Evaluating $\tau^\text{ell}(z+\omega_i)$ is the same as evaluating $\tau^\text{ell}(z)$ where all the $y_i$ are shifted by $-\omega_i$. By the definition of the Drinfeld functor, this is the same as applying $T_1^\vee(y_1)$ and its conjugates. Tracing might give
\begin{equation*}
\tau^\text{ell}(z+\omega_i) = Z_i \tau^\text{ell}(z).
\end{equation*}
This looks similar to theta functions.
We have
\begin{align*}
\tau_\text{ell}(z;y_1,...,y_i+1,...,y_N)
&= \operatorname{tr}_{-1,0} R_{0N}(z-y_N) \cdots R_{0i}(z-y_i-1) \cdots R_{01}(z-y_1) \\
&= \operatorname{tr}_{-1,0} T^\vee_{i,-1}(y_i) R_{0N}(z-y_N) \cdots R_{0i}(z-y_i) \cdots R_{01}(z-y_1) \\
&= f(y_i) K(y_i;a) \tau_\text{ell}(z;y_1,...,y_N).
\end{align*}
We can take $a=0$ and $f(z) = \exp(2\eta_1 (z+\omega_1))/z$ and get the equation
\begin{equation*}
\tau_\text{ell}(z;y_1,...,y_i+2 \omega_1,...,y_N) = \exp(2\eta_1 (y_i+\omega_1)) \sigma_1 \tau_\text{ell}(z;y_1,...,y_N)
\end{equation*}
which is fulfilled by a Weierstraß $\sigma$-function in $y_i$. We can also choose a representation in which $\tau_\text{ell}(z)$ becomes a theta function or elliptic function. Then we can do the expansions from Akhiezer. For example, we can make an expansion
\begin{equation*}
\tau_\text{ell}(z) = \widehat{\chi}(1) \sum_j \phi(w;z-y_j) X_j
\end{equation*}
by matching the quasi-periodicity of $\tau_\text{ell}(z)$ to that of $\phi(w;z)$ and arguing that the relevant quotient is a holomorphic elliptic function.
\end{proof}
...
Let
\begin{equation*}
\Phi(x,z) := \frac{\sigma(x+z)}{\sigma(x)\sigma(z)} \exp(-\zeta(z)x).
\end{equation*}
This has a simple pole at $x=0$ with residue 1 and
\begin{equation*}
\Phi(x,z) = \frac{1}{x} - \frac{1}{2} \wp(z) x + \mathcal{O}(x^2)
\end{equation*}
It is doubly quasi-periodic with
\begin{equation*}
\Phi(x+2\omega_\alpha,z) = \exp(2(\zeta(\omega_\alpha)z-\zeta(z)\omega_\alpha) \Phi(x,z) 
\end{equation*}
