...
\begin{remark}
The matrices $\bar T^\vee(z)$ and $T^\vee(z)$ work two represent a line crossing a seam in a torus diagram. This gives a graphical interpretation to the relations. We may define automorphisms of $DY_0(\mathfrak{gl}_\ell)$ via the topological action by $\widetilde{SL_2(\Z)}$:
\begin{align*}
...
U &\mapsto (DY_0(\mathfrak{gl}_\ell) \otimes V^+[y_1] \otimes \cdots \otimes V^+[y_N]) \otimes_{\widetilde{SL_2(\Z)} \ltimes \ddot S_N} U
\end{align*}
\end{theorem}
