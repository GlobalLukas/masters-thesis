...
If we want to extrapolate to the elliptic case, we have to look at the composition of a coevaluation $[0] \to [2]$ with a pair-of-pants $[2] \to [1]$. The 1-forms we would want to look at on $E = \C/\Lambda$ are
\begin{equation*}
\omega = K \prod_i \frac{\sigma(z-y_i-\eta)}{\sigma(z-y_i)} dz,
\end{equation*}
again with $N$ simple poles and zeros away from the origin, so $y_i,y_i-y_j,y_i-y_j-\eta \notin \Lambda$.

Define a degenerate elliptic Hecke algebra
\begin{equation*}
E\dot H_0 := \widetilde{SL_2(\Z)} \ltimes \C[\lambda][\lambda^{-1},(\lambda-1)^{-1}]
\end{equation*}
where $\lambda$ is the $\lambda$-function, so that $j=256 \frac{(\lambda^2-\lambda+1)^3}{\lambda^2(\lambda-1)^2}$ is the $j$-invariant. Then $S \cdot \lambda = 1-\lambda$ and $T \cdot \lambda = \frac{\lambda}{\lambda-1}$. The algebra $E\dot H_0$ has to have a map from $\dot H_1 \otimes \dot H_2$. This would provide three complex numbers $y,y_1,y_2$, which could give the branch points defining the elliptic curve:
\begin{equation*}
\dot H_1 \otimes \dot H_2 \to E\dot H_0, \quad \wp(y) \mapsto \lambda, \wp(y_1) \mapsto 0, \wp(y_2) \mapsto 1, 1 \otimes s_2 \mapsto S.
\end{equation*}
We would then expect
\begin{equation*}
U \in \mathsf{Qcoh}(M_{1,1}) \mapsto EY(\mathfrak{gl}_\ell) \otimes_{\widetilde{SL_2(\Z)}} U.
\end{equation*}
