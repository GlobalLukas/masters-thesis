...
\begin{equation*}
\langle \cdot,\cdot \rangle: Y(\mathfrak{gl}_\ell) \otimes Y^\vee(\mathfrak{gl}_\ell) \to \C[\eta], \quad t_{ij}^{(r)} \otimes t_{kl}^{(-s)} \mapsto -\delta_{rs} \delta_{il} \delta_{jk}
\end{equation*}
...
We will use this in conjunction with the results of \cite{article:benzvi:2018} to construct a 2-functorial field theory on $\mathcal{ES}$ via factorization homology. To accomplish this, we need a balanced braided abelian tensor category as the $E_2$-algebra of coefficients. Such a category is readily provided by locally finite-dimensional $Y(\mathfrak{gl}_\ell)$-modules:

\begin{definition}
Consider $\eta$ as a non-zero complex number and let $\Omega \subseteq \C$ be a subset that fulfills $y_1-y_2 \neq \pm\eta$ for any $y_1,y_2 \in \Omega$. Let $Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\Omega$ be the subcategory generated by $L(\ydiagram{1})_y^t$ for $y \in \Omega$ under the rigid monoidal structure and finite (co)limits.
\end{definition}

\begin{example}
Let $\Omega = \{ z \in \C \mid |\Im z|<|\eta| \}$ and $\eta$ be purely imaginary.
\end{example}

\begin{prop}
$Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\Omega$ is a symmetric rigid monoidal abelian category.
\end{prop}

\begin{proof}
$Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\Omega$ is rigid monoidal abelian by construction. Further recall that we have a braiding
\begin{equation*}
\check R(y_1-y_2): L(\ydiagram{1})_{y_1}^t \otimes L(\ydiagram{1})_{y_2}^t \to L(\ydiagram{1})_{y_2}^t \otimes L(\ydiagram{1})_{y_1}^t
\end{equation*}
that fulfills the symmetry condition by unitarity. It is well defined and invertible due to $y_1-y_2 \neq \pm \eta$. We can equip the rest of $Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\Omega$ with a braided rigid monoidal structure by taking finite (co)limits, since tensoring with $L(\ydiagram{1})_y^t$ is exact.
\end{proof}

\begin{prop}
The completed dual Yangian $\hat Y^\vee(\mathfrak{gl}_\ell)$ is the coend $\int^X X \otimes X^\vee$, where $X$ ranges over $Y(\mathfrak{gl}_\ell)\mathsf{Mod}^\Omega$.
\end{prop}
