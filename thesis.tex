...
\begin{definition}
The \emph{dual Yangian}\index{Dual Yangian} for the complex Lie algebra $\mathfrak{gl}_\ell$, written $Y^\vee(\mathfrak{gl}_\ell)$, is defined as the complex associative algebra with the following presentation: It has generators $t_{ij}^{(-r)}$ for $1 \leq i,j \leq \ell$ and $r \geq 1$. To describe the defining relations, we again let $t_{ij}^{(0)} = \delta_{ij}/\eta$ and introduce
\begin{equation*}
...
\end{equation}
We may equip the dual Yangian with a descending filtration by giving $t_{ij}^{(-r)}$ the degree $r$. Denote the $N$th filtered piece by $F_N Y^\vee(\mathfrak{gl}_\ell)$ and the completion with respect to this filtration by $\widehat Y^\vee(\mathfrak{gl}_\ell)$.
\end{definition}
...
\begin{remark}
The bialgebra structure of $Y^\vee(\mathfrak{gl}_\ell)$ extends to the completion $\widehat Y^\vee(\mathfrak{gl}_\ell)$. We may additionally define an antipode on $\widehat Y^\vee(\mathfrak{gl}_\ell)$, see \cite{article:nazarov:2019}, making $\widehat Y^\vee(\mathfrak{gl}_\ell)$ into a Hopf algebra.
\end{remark}
...
\end{equation}
define an element $\mathcal{R} \in Y^\vee(\mathfrak{gl}_\ell) \operatorname{\widehat \otimes} Y(\mathfrak{gl}_\ell)$ called \emph{the universal $R$-matrix}.
\end{definition}
...
\begin{equation*}
\Pi_{t_{[2,1^{k-1}]}} = \frac{1}{(k+1)!} (k+Y_{k+1}) \prod_{i=1}^k (1-Y_i),
\end{equation*}
...
\end{equation}
consistent with the Littlewood-Richardson rule.
\end{example}

\begin{theorem}
The fusion projectors form a complete set of primitive orthogonal idempotents on $(\C^\ell)^{\otimes k}$ such that
\begin{equation*}
L_{t_\lambda} := \Pi_{t_\lambda} (V_{y-\eta c_1}^+ \otimes \cdots V_{y-\eta c_k}^+)
\end{equation*}
is a $Y(\mathfrak{gl}_\ell)$-submodule of $V_{y-\eta c_1}^+ \otimes \cdots V_{y-\eta c_k}^+$ isomorphic to $L(\lambda)_y^t$.
\end{theorem}
...
\end{equation*}
But $\Pi_{t_{[1^\ell]}} (\gamma_{i_1} e_{i_1} \otimes \cdots \otimes \gamma_{i_\ell} e_{i_\ell})$ is only non-zero when the $i_1,...,i_\ell$ define a permutation $\sigma$, in which case it reduces to $\gamma_1 \cdots \gamma_\ell \cdot \operatorname{sgn} \sigma \cdot (e_1 \otimes \cdots \otimes e_\ell)$.
\end{proof}
...
The functional relations between higher transfer matrices come from the so called \emph{fusion relations}\index{Fusion procedure}, whose categorification are certain short exact sequences of comodules. Of particular importance will be the following short exact sequence:
\begin{equation*}
...
\begin{proof}
This is just the canonical partial fraction decomposition of $\tau_\lambda^g(z)$. To show that this is indeed the case, we need to show that $\tau_\lambda^g(z)$ only has poles at $y_1,...,y_N$ and that $\tau_\lambda^g(\infty) = \chi_\lambda(g)$. (tba) Let us take the column tableau $t_\lambda$, giving
\begin{equation*}
...
\begin{proof}
We have $\operatorname{qdet} T(z) = \prod_k \frac{z-y_k-\eta}{z-y_k}$, so that $\operatorname{Res}_{z=y_j} \operatorname{qdet} T(z) = -\eta \prod_{j \neq k} \frac{y_j-y_k-\eta}{y_j-y_k}$. On the other hand, we have $L = CH \Leftrightarrow H = (-L^t) (-C^t)^{-1}$, where $C_{ij} = \frac{\eta}{y_i-y_j+\eta}$ is the Cauchy matrix and $H = \operatorname{diag}(H_1,...,H_N)$. Iterating the recursion relation from lemma \ref{lemma:recursionRelation} until we reach the quantum determinant $\tau_{[1^\ell]}^g(z) = \det g \operatorname{qdet} T(z)$ yields
\begin{equation*}
\chi_{[1^\ell]}(g)
\begin{pmatrix}
...
which implies
\begin{equation*}
\left( \sum_{k=0}^\ell \chi_{[1^{\ell-k}]}(g) (-L^t)^k \right) (C^t)^{-1} e = 0
\end{equation*}
due to $H e = (-L^t) (-C^t)^{-1} e$, where $e = (1,...,1)$, so that $(C^t)^{-1} e$ is the vector from (\ref{equation:spectralEq})
\end{proof}

\begin{remark}
Recall that $\chi_{[1^k]}(g) = e_k(\gamma_1,...,\gamma_\ell)$. It becomes clear that the Cayley-Hamilton form of the spectral equation is consistent with the findings of \cite{article:gorsky:2014}, where it is shown that the twist parameters $\gamma_1,...,\gamma_\ell$ give the eigenvalues of the Lax matrix $L$ with multiplicities given by the eigenvalues of the occupation number operators\index{Occupation number operators}
...
%\end{equation*}
let us take the generalization
\begin{equation*}
\mathbf{H}_i := R_{\lambda^{(i)}\lambda^{(i-1)}}(y_i-y_{i-1}) \cdots R_{\lambda^{(i)}\lambda^{(1)}}(y_i-y_1) g_i R_{\lambda^{(i)}\lambda^{(N)}}(y_i-y_N) \cdots R_{\lambda^{(i)}\lambda^{(i+1)}}(y_i-y_{i+1})
\end{equation*}
where $R_{\lambda\mu}(z-w)$ denotes the higher $R$-matrix (\ref{eq:higherRMatrix}) for the Young diagram $\lambda^{(i)}$ of $k$ boxes with a chosen standard Young tableau. These are the \emph{higher non-local Hamiltonians}. %Thus, we find that $\tau^g(z)$ obtains poles at $z = y_i-\eta c_j$, but these in general might not be simple. The relevant Hamiltonians then are the coefficients of the principal parts at the poles $z = y_i-\eta c_j$. In general, these have the form
%\begin{align*}
...
On the particle side, we may similarly introduce \emph{higher $x$-operators}
\begin{equation*}
x_{\lambda \mu} := \overleftarrow{\prod_i} \overrightarrow{\prod_j} x_{ij} \in \dot H_{k+l}
\end{equation*}
...
\end{equation*}
Letting $\mathbf{\check H}_i$ denote the higher non-local Hamiltonian $\mathbf{H}_i$ with unitary normalizations, we obtain the following correspondence:
...
\end{equation*}
for the $\ydiagram{4}$-multiplet, which is exactly correct (compare \cite{book:arutyunov:betheAnsatz}, section 8.7) up to the prefactor $(z-\eta)^2$, which results from the polynomial convention used in \emph{loc. cit.}
\end{example}
...
\end{equation*}
with the special element $1_N: \C \to E_N$ representing the sum of identities. By the previous lemma, we can identify $F_N Y(\mathfrak{gl}_\ell)$ as a subspace of $E_N$ via the map $Y(\mathfrak{gl}_\ell) \to E_N$ induced by the jointly faithful actions. Further let $L := \int^{V \in Y(\mathfrak{gl}_\ell)\mathsf{Mod}} V \otimes V^\vee$. The coactions of the dual Yangian give a map $\psi: L \to Y^\vee(\mathfrak{gl}_\ell)$ that commutes with the coactions $E_N \to L \otimes E_N$ and $E_N \to Y^\vee(\mathfrak{gl}_\ell) \otimes E_N$. We may look at the image of $1_N$
\begin{equation*}
\widetilde{\mathcal{R}}_N: \C \to E_N \to L \otimes F_N Y(\mathfrak{gl}_\ell) \subseteq L \otimes E_N,
\end{equation*}
...
\end{equation*}
mapping to the universal $R$-matrix $\mathcal{R}$. The element $\widetilde{\mathcal{R}}$ then witnesses that the composition $Y(\mathfrak{gl}_\ell) \otimes L \to Y(\mathfrak{gl}_\ell) \otimes Y^\vee(\mathfrak{gl}_\ell) \to \C$ is a non-degenerate pairing, implying that $\psi$ is an isomorphism. tba: Resolve topological complications
\end{proof}
