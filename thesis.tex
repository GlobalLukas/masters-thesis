...
\begin{prop}
There is a double functor $\mathsf{STCob} \to \C[\eta]\mathsf{Alg}$ mapping a pointed set $[N]$ to $Y(\mathfrak{gl}_\ell)^{\otimes N}$, a pointed permutation $\sigma: [N] \to [N]$ to the corresponding permutation on tensor factors, a genus zero curve $(\P^1;\infty,y_1,...,y_N;-\partial_{1/z},v_1,...,v_N)$ to the fundamental monodromy representation $L(\ydiagram{1})_{y_1}^t \otimes \cdots \otimes L(\ydiagram{1})_{y_N}^t$, shifts as
\begin{center}
\begin{tikzcd}[column sep=3.5cm]
{[1]} \arrow{d}{\id} \arrow[dashed]{r}{(\P^1;\infty,y;-\partial_{1/z},v)} & {[0]} \arrow{d}{\id} \\
{[1]} \arrow[dashed,swap]{r}{(\P^1;\infty,y';-\partial_{1/z},v')} & {[0]}
\end{tikzcd}
$\quad \mapsto \quad$
\begin{tikzcd}[column sep=4cm]
Y(\mathfrak{gl}_\ell) \arrow{d}{e^{(y'-y)\partial}} \arrow[dashed]{r}{L(\ydiagram{1})_{y}^t} & {\C[\eta]} \arrow{d}{\id} \\
Y(\mathfrak{gl}_\ell) \arrow[dashed,swap]{r}{L(\ydiagram{1})_{y'}^t} & {\C[\eta]}
\end{tikzcd}
\end{center}
with the identity map, braids as
\begin{center}
\begin{tikzcd}[column sep=3.5cm]
{[2]} \arrow{d}{(1 \ 2)} \arrow[dashed]{r}{(\P^1;\infty,y_1,y_2;-\partial_{1/z},v_1,v_2)} & {[0]} \arrow{d}{\id} \\
{[2]} \arrow[dashed,swap]{r}{(\P^1;\infty,y_1,y_2;-\partial_{1/z},v_1,v_2)} & {[0]}
\end{tikzcd}
$\quad \mapsto \quad$
\begin{tikzcd}[column sep=4cm]
Y(\mathfrak{gl}_\ell)^{\otimes 2} \arrow{d}{(1 \ 2)} \arrow[dashed]{r}{L(\ydiagram{1})_{y_1}^t \otimes L(\ydiagram{1})_{y_2}^t} & {\C[\eta]} \arrow{d}{\id} \\
Y(\mathfrak{gl}_\ell)^{\otimes 2} \arrow[dashed,swap]{r}{L(\ydiagram{1})_{y_1}^t \otimes L(\ydiagram{1})_{y_2}^t} & \C[\eta]
\end{tikzcd}
\end{center}
with the $R$-matrix $\check R_{12}(y_1-y_2)$, and twists as
\begin{center}
\begin{tikzcd}[column sep=3.5cm]
{[1]} \arrow{d}{\id} \arrow[dashed]{r}{(\P^1;\infty,y;-\partial_{1/z},v)} & {[0]} \arrow{d}{\id} \\
{[1]} \arrow[dashed,swap]{r}{(\P^1;\infty,y;-\partial_{1/z},v)} & {[0]}
...
$\quad \mapsto \quad$
\begin{tikzcd}[column sep=4cm]
Y(\mathfrak{gl}_\ell) \arrow{d}{\operatorname{ad}_g} \arrow[dashed]{r}{L(\ydiagram{1})_{y}^t} & {\C[\eta]} \arrow{d}{\id} \\
Y(\mathfrak{gl}_\ell) \arrow[dashed,swap]{r}{L(\ydiagram{1})_{y}^t} & {\C[\eta]}
\end{tikzcd}
\end{center}
with the matrix $g$.
...
\begin{proof}
The functor $F^0: \mathsf{STCob}^0 \to \C[\eta]\mathsf{Alg}^0$ is easily defined by mapping $[N]$ to $Y(\mathfrak{gl}_\ell)^{\otimes N}$ and a pointed bijection $\sigma: [N] \to [N]$ to the corresponding permutation on tensor factors.

The functor $F^1: \mathsf{STCob}^1 \to \C[\eta]\mathsf{Alg}^1$ is harder to define: Note that the space of loose morphisms $[N] \dashrightarrow [M]$ is nothing but the complex Teichmüller groupoid $\mathcal{T}eich_{[N] \sqcup [M]}^\C$. By theorem 6.1.12 in \cite{book:bakalov}, this is equivalent to the standard Teichmüller groupoid, which in turn has a functor to the fundamental groupoid of the markings complex defined in \cite{article:bakalov:1998}. Hence it suffices to define a functor from the fundamental groupoid of the markings complex to the category of $Y(\mathfrak{gl}_\ell)^{\otimes N}$-$Y(\mathfrak{gl}_\ell)^{\otimes M}$-bimodules.
\end{proof}
