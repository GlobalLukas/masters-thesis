...
\item The $\C[\eta]$-subalgebra $\dot H_N \subset \ddot H_N$ generated only by $\C[S_N]$ and $\C[y_1,...,y_N]$ is the \emph{degenerate affine Hecke algebra (of type $A_{N-1}$)}\index{Degenerate affine Hecke algebra}. It is also generated by the $s_i$ and $y_1$. There is an evaluation morphism
\begin{equation*}
...
We now take a closer look at how transfer matrices act on monodromy representations. Let us introduce the notation $V_y^- := L(\ydiagram{1})_y$ for the vector representation, and $V_y^+ := L([1^{\ell-1}])_y \cong L(\ydiagram{1})_y^t$ for the covector representation. Our starting observation is the following:
...
Let $t_\lambda$ be a standard Young tableau of shape $\lambda$ with content vector $(c_1,...,c_k)$ and consider $(\C^\ell)^{\otimes k}$ under the $Y(\mathfrak{gl}_\ell)$-action of the monodromy representation
\begin{equation*}
...
\begin{equation*}
V_0^+ \otimes V_{-\eta}^+ \otimes \cdots \otimes V_{-\eta(k-1)}^+ \otimes V_1^+.
\end{equation*}
...
\begin{equation*}
D_{\ell,N}(\C[y_1,...,y_N]) \otimes_{\C[y_1,...,y_N]} \C[y_1,...,y_N]/\mathfrak{m} \cong V_{\bar y_1}^+ \otimes \cdots \otimes V_{\bar y_N}^+
\end{equation*}
...
On the other hand, we may equivalently try to apply the fusion procedure, taking the limit $y_i \to y + \eta c_i$ and projecting onto the subspace $L_{t_\lambda} \subset V_{y+\eta c_1}^+ \otimes \cdots \otimes V_{y+\eta c_N}^+$ isomorphic to $L(\lambda)_y^t$.
...
\begin{center}
\begin{tikzcd}
Y(\mathfrak{gl}_\ell)_k \arrow{r} \arrow{dr} & L \otimes Y(\mathfrak{gl}_\ell)_k \arrow{d}{\psi_k \otimes \id} \\
& Y^\vee(\mathfrak{gl}_\ell)_k \otimes Y(\mathfrak{gl}_\ell)_k
\end{tikzcd}
\end{center}
