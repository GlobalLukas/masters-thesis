...
\begin{enumerate}[label=(\roman*)]
\item objects are natural numbers $[N]$, thought of as a disjoint union of $N$ formal disks $D := \operatorname{Spec} \C[\eta]\llbracket z \rrbracket$,
...
\begin{definition}
Let $\C\mathsf{Cob}$ denote the monoidal $2$-category whose
\begin{enumerate}[label=(\roman*)]
\item objects are natural numbers $[N]$,
\item 1-morphisms $[N] \to [1]$ are quasi-coherent sheaves on $M_{0,N+1}$,
\item 2-morphisms are morphisms of quasi-coherent sheaves,
\item tensor product is tensor product of sheaves.
\end{enumerate}
\end{definition}
...
\end{equation*}
where $\sigma(f)$ is defined using the natural action of $S_N \times \C$ on $\C[y_1,...,y_N]_\delta$ by permutations and shifts.
\end{definition}
...
\begin{theorem}
There is a monoidal $(2,1)$-functor
\begin{equation*}
\mathcal{Z}_0: \C\mathsf{Cob}_0 \to \mathsf{Rex}
\end{equation*}
that sends
...
\item 1-morphisms $U: [N] \to [1]$, which are modules over $S_N \ltimes \C[y_1,...,y_N]_\delta$, to the functor
\begin{equation*}
...
Previously, composition of 1-morphisms was defined using the gluing map
\begin{equation*}
\gamma: \overline{M}_{0,2+1} \times \overline{M}_{0,N_1+1} \times \overline{M}_{0,N_2+1} \to \overline{M}_{N_1+N_2+1}.
\end{equation*}

Given sheaves $U_i$ on $\overline{M}_{0,N_i+1}$, we may form the external product $\mathcal{O}_{\overline{M}_{0,2+1}} \boxtimes U_1 \boxtimes U_2$, which is a sheaf on $\overline{M}_{0,2+1} \times \overline{M}_{0,N_1+1} \times \overline{M}_{0,N_2+1}$, and push forward along $\gamma$ to obtain the composition. This is nothing but the external tensor product of $\dot H_N$-modules we looked at previously.
