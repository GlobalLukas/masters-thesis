...
\title{
\Large \textsc{Quantum-classical Duality between \\ Heisenberg and Ruijsenaars-Schneider Models}
}
\author{
~\\
...
%I sincerely want to thank my supervisor Prof. Dr. Gleb Arutyunov for his feedback and guidance in the research process and for sparking my interest in this wonderful topic. I also thank (tba) for proofreading the text as well as (tba) Finally, I would like to thank my family for supporting me in my studies.
...
\begin{tikzpicture}[remember picture,overlay]
  \node[opacity=0.1] at (7.45cm,-4.9cm) {
    \includegraphics[width=11cm, height=7.5cm]{Butterfly}
  };
\end{tikzpicture}
\begin{center}
...
The aim of this section is to reformulate Schur-Weyl duality for the Yangian in terms of a 2-functorial quantum field theory that will turn out to represent four-dimensional Chern-Simons theory. The mathematical structures used so far have been heavily representation theoretical. Nonetheless, we have seen some hints that there is an underlying geometry at play: We have made use of braid diagrams on cylinders as well as complex coordinates that may be reinterpreted as $N$-pointed Riemann spheres. The foundation for our construction is the observation that $\ddot H_N$-modules may be seen as local systems on $M_{0,[N]}$. Let us unravel what this means.

\begin{definition}
\begin{enumerate}[label=(\roman*)]
\item The \emph{category of pointed sets} has as objects sets with a distinguished element and maps that preserve distinguished elements. Canonical representatives of the isomorphism classes are given by $[N] := \{ \infty,1,...,N \}$ with $\infty$ distinguished.
...
\end{equation*}
consists of a pointed bijection $b: A \to A$ as well as a morphism of complex projective curves $m: C \to C'$ such that $m(y_a) = y_{b(a)}'$ and $dm_{y_a}(v_a) = v_{b(a)}'$.
...
\begin{equation*}
M_{0,[N]} \cong \C \backslash (\text{Conf}_N \times (\C^\times)^N)/S_N,
\end{equation*}
where we have quotiented by the action of $\C$ by translation and $S_N$ by permutation of factors.
\end{example}

\begin{definition}
A \emph{local system on $M_{0,[N]}$} is a functor $\mathcal{T}eich_{0,[N]}^\C \to \mathsf{Vect}$. Together with natural transformations, they define a category $\mathsf{Loc} M_{0,[N]}$.
\end{definition}

\begin{prop}
The local automorphism groups of $\mathcal{T}eich_{0,[N]}^\C$ are $\dot B_N := B_N \ltimes \Z^N$.
\end{prop}

\begin{proof}
It is known that the mapping class group of the standard $(1+N)$-pointed sphere is generated by $b_i, 0 \leq i \leq N-1$, braiding the punctures labeled by $i$ and $i+1$, and Dehn twists $t_i, 0 \leq i \leq N$ around the punctures, as well as a rotation $z$ sending the $i$th puncture to the $(i+1)$st puncture for $0 \leq i \leq N-1$ and sending the $N$th to the $0$th puncture. Their complete set of relations is given by
\begin{align*}
b_ib_j = b_j b_i,|i-j|>1 \quad b_i X_j = X_j b_i,|i-j|>1,i=j+1 \quad b_i b_{i+1} b_i = b_{i+1} b_i b_{i+1}, \\ b_i^{\pm 1} X_i = X_{i+1} b_i^{\pm 1}, \quad X_i X_j = X_j X_i, \quad z^{N+1} = 1, \quad b_0 \cdots b_{N-1} X_N = z, \quad z X_N = X_0 z.
\end{align*}
Now, we are working with \emph{pointed} sets, meaning the smooth families of curves representing morphisms in the Teichmüller groupoid are required to fix the distinguished point. This eliminates $b_0,X_0$ and $z$ from the set of generators and we are left with the affine braid group $B_N \ltimes \Z^N$ as the mapping class group of the $[N]$-pointed sphere.
\end{proof}

What happens to the shift automorphism??

\begin{corollary}
We have an equivalence of categories $\mathsf{Loc} M_{0,[N]} \simeq \dot B_N \mathsf{Mod}$, yielding the functor
\begin{equation*}
\ddot H_N \mathsf{Mod} \to \dot S_N \mathsf{Mod} \to \dot B_N \mathsf{Mod} \simeq \mathsf{Loc} M_{0,[N]}.
\end{equation*}
\end{corollary}
...
\begin{equation*}
\bigoplus_N \ddot H_N \mathsf{Mod} \to \mathsf{Loc} M_{0,[*]},
\end{equation*}
where the monoidal structure on the right hand side is defined by gluing curves somehow.
\end{prop}
...
\begin{prop}
The fundamental monodromy representations define a functor
\begin{equation*}
M_{0,[N]} \to Y(\mathfrak{gl}_\ell)\mathsf{Mod}, \quad (\P^1;\infty,y_1,...,y_N;-\partial_{1/z},v_1,...,v_N) \mapsto L(\ydiagram{1})_{y_1}^t \otimes \cdots \otimes L(\ydiagram{1})_{y_N}^t.
\end{equation*}
The permutation generators get mapped to the $R$-matrix. There is a shift functor on both sides that gets intertwined.
\end{prop}
...
\begin{prop}
The fundamental monodromy representations define a local system
\begin{equation*}
\mathcal{T}eich_{0,[N]} \to \mathsf{Vect}, \quad (\P^1;\infty,y_1,...,y_N;-\partial_{1/z},v_1,...,v_N) \mapsto (\C^\ell)^{\otimes N} \otimes \ddot H_1^{\otimes N}.
\end{equation*}
The braid generators get mapped to the $R$-matrix while the twist generators act on the $\ddot H_1$ factors.
\end{prop}

Can this be generalized to other $\ddot H_N$-modules?

I have three types of morphisms for the curves: Permutations of points, morphisms of curves, families of curves. For the algebras, I have three types: modules, algebra morphisms, module maps.

\begin{tabular}{ll}
point permutation & module map/algebra morphism \\
morphism of curves & algebra morphism \\
family of curves & module map \\
curve & module
\end{tabular}

Make use of the $N$-truncated Yangian?

\pagebreak

Let us now combine all these data into one big symmetric monoidal double category $\C\mathsf{Cob}_{0,2,4}$. Regarding symmetric monoidal double categories we follow the reference \cite{article:hansen:2019}, where they were originally defined as a tool for efficiently organizing the structure of symmetric monoidal bicategories.
