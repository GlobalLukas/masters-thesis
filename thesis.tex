...
%Let us also remark that these relations admit an anti-automorphism
%\begin{equation*}
...
\begin{definition}
The \emph{quantum trigonometric Calogero-Moser model} is given by the latter polynomial representation of the degenerate double affine Hecke algebra\index{Quantum trigonometric Calogero-Moser model}, \emph{i.e.} $\C[X_1^\pm,...,X_N^\pm]$ is its Hilbert space of wave functions with position operators $X_1,...,X_N$ and momentum-like operators $y_1,...,y_N$. We again have a set of commuting Hamiltonians in the spherical degenerate double affine Hecke algebra given by
\begin{equation*}
...
L(\lambda^{(1)})_{y_1}^t \otimes \cdots \otimes L(\lambda^{(N)})_{y_N}^t,
\end{equation*}
as above are called \emph{monodromy representations}\index{Monodromy representation} with \emph{inhomogeneities} $y_1,...,y_N$.
\end{definition}
...
\begin{remark}
This shows that the RTT relation (\ref{equation:RTTRelation}) in the representation $V_y^+$ becomes the \emph{quantum Yang-Baxter equation}\index{Quantum Yang-Baxter equation}
\begin{equation*}
...
\end{equation*}
which is represented by the $Y(\mathfrak{gl}_\ell)$-module $V' \otimes V$. We conclude that the (co)algebra corresponding to $D^\times \times S_*^1$ with $V$ inserted at the basepoint is the coalgebra $V \otimes V^\vee$. To get rid of the dependence on $V$, we should take the coend
\begin{equation*}
...
\end{align*}
This defines a $Y(\mathfrak{gl}_\ell)$-$\underline{\dot H}_\Gamma$-bimodule structure on $E_\Gamma$, where the permutation $(i \ j)$ acts via the braiding $P_{ij} \check R_{ij}^E(y_i-y_j)$ and the polynomial generators act via multiplication. In particular, $P_{ij} \check R_{ij}^E(y_i-y_j)$ becomes an $(i \ j)$-equivariant map of $Y(\mathfrak{gl}_\ell)$-$\underline{\dot H}_\Gamma$-bimodules.
...
\end{align*}
where $Z_0$ maps $\sigma \in \dot S_N$ to the permutation of tensorands determined by $\bar \sigma \in S_N$, and $Z_1$ maps
\begin{enumerate}[label=(\roman*)]
...
\item $(X_i,f): (\Gamma,U) \to (\Gamma,U')$ to $P_w \check H_i \otimes f$, where $P_w$ denotes the permutation determined by the Coxeter element $w$ and $\check H_i$ denotes the $i$th non-local Hamiltonian.
\end{enumerate}
%squares
%\begin{center}
%\begin{tikzcd}
%{[2]} \arrow{r}{(\Gamma,U)} \arrow{d}{s_1} & {[1]} \arrow{d}{\id} \\
%{[2]} \arrow{r}{(\Gamma,U')} & {[1]}
%\end{tikzcd}
%\end{center}
%labeled by an $s_1$-equivariant $f: U \to U'$ to
%\begin{equation*}
%bla
%\end{equation*}
%and squares
%\begin{center}
%\begin{tikzcd}
%{[N]} \arrow{r}{(\Gamma,U)} \arrow{d}{X_i} & {[1]} \arrow{d}{\id} \\
%{[N]} \arrow{r}{(\Gamma,U')} & {[1]}
%\end{tikzcd}
%\end{center}
%labeled by an $\id$-equivariant $f: U \to U'$ to $f \otimes P_w \check H_i$.
\end{theorem}

\begin{proof}
\begin{enumerate}[label=(\roman*)]
\item Well-definition: It is immediate that $Z_0$ is well-defined. It remains to check that $Z_1$ maps the morphisms of $\mathbb{C}\textnormal{ob}_1$ to well-defined $Y(\mathfrak{gl}_\ell)$-linear maps of bicomodules. Indeed, $((i \ j),f)$ gets mapped to $(i \ j) \otimes f$, which is a well-defined $Y(\mathfrak{gl}_\ell)$-linear map since both $(i \ j)$ and $f$ are $(i \ j)$-equivariant. Similarly, we know that $(X_i,f)$ gets mapped to $P_w \check H_i \otimes f$, which is a well-defined $Y(\mathfrak{gl}_\ell)$-linear map since both maps are $\id$-equivariant and $Y(\mathfrak{gl}_\ell)$-linear.
...
\begin{lemma}
Let $E,E'$ be $A$-$B$-bimodules and $U,U'$ be $B$-modules. Let $B$ have an action of a group $G$ and let $h: E \to E'$ and $f: U \to U'$ be $g$-equivariant for some $g \in G$. Then $h \otimes f$ is a well-defined $A$-linear map.
\end{lemma}

\begin{proof}
We check
\begin{align*}
(h \otimes f)(e \otimes bu)
= h(e) \otimes f(bu)
= h(e) \otimes g(b)f(u)
= h(e)g(b) \otimes f(u)
= h(eb) \otimes f(u)
= (h \otimes f)(eb \otimes u)
\end{align*}
as well as
\begin{equation*}
(h \otimes f)(ae \otimes u)
= h(ae) \otimes f(u)
= ah(e) \otimes f(u)
= a(h \otimes f)(e \otimes u).
\end{equation*}
\end{proof}
